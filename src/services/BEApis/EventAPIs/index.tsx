...
export const apiGetAllEventsForAUser = (appUserId: string) => {
  const response = apiInstance.get(`event/user-events?appUserId=${appUserId}`);
  console.log("API RES", response);
  return response;
};
export const apiPostRequestHelp = (appUserId: string) => {
  const response = apiInstance.get(
    `/help/create-ticket/:keyAppUserId=${appUserId}`
  );
  console.log("API RES", response);
  return response;
};
